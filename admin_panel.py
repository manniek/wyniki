...
def show_panel(wiersz_ucznia):
    ...
    # 3. TWOJA LOGIKA PRZETWARZANIA (Z TWOJEGO APP.PY)
    df_temp = wiersz_ucznia.copy()
    nowe_nazwy = []
    for col in df_temp.columns:
        czesci_col = [str(p) for p in col if "Unnamed" not in str(p)]
    ...
    # 4. TABELA WYNIKÓW (TWOJA TABELA HTML)
